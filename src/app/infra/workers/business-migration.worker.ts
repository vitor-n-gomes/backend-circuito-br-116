...
export class BusinessMigrationWorker implements IDataMigrationWorker {
  ...
  async run(): Promise<DataMigrationResult> {
    ...
    this.logger.log('🚀 Starting Business migration from MySQL to PostgreSQL...');
      ...
      for (let i = 0; i < batches; i++) {
        ...
        for (const legacy of legacyBusinesses) {
          try {
            ...
            // Check if already processed in this run
            if (processedKeys.has(uniqueKey)) {
              this.logger.debug(`⏭️  Skipping duplicate in batch: ${legacy.companyName}`);
              result.skipped++;
              continue;
            }
            ...
            const existing = await this.businessRepo.findOne({
              where: [
                { title: legacy.companyName },
                ...(legacy.email ? [{ email: legacy.email }] : []),
              ],
            });

            if (existing) {
              this.logger.debug(`⏭️  Skipping existing: ${legacy.companyName}`);
              result.skipped++;
              processedKeys.add(uniqueKey);
              continue;
            }
            ...
            this.logger.debug(`✅ ${this.isDryRun ? '[DRY RUN] ' : ''}Imported: ${legacy.companyName}`);
          } catch (error) {
            result.failed++;
            const errorMsg = `Failed to import ${legacy.companyName}: ${error.message}`;
            result.errors.push(errorMsg);
            this.logger.error(`❌ ${errorMsg}`);
          }
        }
