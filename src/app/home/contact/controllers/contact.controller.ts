...
export class ContactController {
  constructor(
    private readonly paginateContactsByFilterCase: PaginateContactsByFilterCase,
    private readonly findContactByIdCase: FindContactByIdCase
  ) {}

  @Get()
  @ApiResponse({
    status: 200,
    type: PaginationResponseDto<ContactResponseDto[]>,
  })
  async listContacts(
    @Query("filter[city]", new ValidationPipe({ transform: true }))
    city: string,
    @Query("filter[query]", new ValidationPipe({ transform: true }))
    query: string,
